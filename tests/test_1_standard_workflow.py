...
from bifrost_ariba_virulencefinder import launcher as bavl
...
def test_install_component(mydb):
    ...
    args: argparse.Namespace = bavl.parser(["--install"])
    bavl.run_program(args)
...
def test_pipeline(mydb):
    ...
    args = bavl.parser(["-id", sample_id,"-out", "/bifrost_test_data/output/test1"])
    bavl.run_program(args)
    assert os.path.isfile("/bifrost_test_data/output/test1/ariba_virulencefinder/datadump_complete")